...
export function AdminDashboard() {
  ...
  if (!user) {
    return (
      <div className="rounded-3xl border border-white/10 bg-surface-container/60 p-8 text-center">
        <p className="text-white/70">Sign in to access admin tools.</p>
  ...
  if (user && allowed === null) {
    return <p className="text-white/50">Verifying admin access…</p>;
  }

  if (user && allowed === false) {
    return (
      <div className="rounded-3xl border border-white/10 bg-surface-container/60 p-8">
  ...
  return (
    ...
    <div className="grid gap-10 lg:grid-cols-[1fr_1.1fr]">
      <section className="rounded-[28px] border border-white/8 bg-surface-container/50 p-6 backdrop-blur-xl">
        <div className="mb-4 flex items-center justify-between gap-4">
          <h2 className="font-headline text-xl font-bold text-white">Catalog</h2>
            ...
            {products.map((p) => (
              <li key={p.id}>
                ...
                >
                  ...
                  <div className="text-xs text-white/45">{p.category} · ${p.price}</div>
                </button>
              </li>
            ))}
          </ul>
      ...
      <section className="rounded-[28px] border border-white/8 bg-surface-container/50 p-6 backdrop-blur-xl">
        <h2 className="font-headline text-xl font-bold text-white">{editingId ? "Edit product" : "Create product"}</h2>
        ...
        <form className="mt-6 space-y-4" onSubmit={(e) => void handleSubmit(e)}>
          ...
          <label className="grid gap-1 text-xs text-white/55">
            Description
            <textarea
              className="min-h-[100px] w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-sm text-white outline-none focus:border-primary/40"
          ...
          <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white/80">
            <input
              type="checkbox"
      ...
      <section className="mt-10 rounded-[28px] border border-white/8 bg-surface-container/50 p-6 backdrop-blur-xl">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
          <h2 className="font-headline text-xl font-bold text-white">Affiliate clicks</h2>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          <div>
            <h3 className="mb-2 text-sm font-semibold text-white/70">Trending product IDs</h3>
            {trending.length === 0 ? (
              <p className="text-sm text-white/40">No clicks in the sampled window.</p>
            ) : (
              <ol className="space-y-2 text-sm">
                {trending.map(([pid, n]) => (
                  <li key={pid} className="flex justify-between gap-4 rounded-xl border border-white/8 bg-black/20 px-3 py-2">
          ...
          <div>
            <h3 className="mb-2 text-sm font-semibold text-white/70">Latest events</h3>
            <ul className="max-h-64 space-y-2 overflow-y-auto text-xs text-white/55">
              {clickRows.slice(0, 15).map((row) => (
                <li key={row.id} className="rounded-lg border border-white/6 bg-black/15 px-2 py-1.5 font-mono">
                  <span className="text-white/80">{row.productId}</span>
                  {row.userId ? <span className="text-tertiary"> · {row.userId.slice(0, 8)}…</span> : null}
                  {row.at ? <div className="text-white/35">{row.at}</div> : null}
                </li>
              ))}
            </ul>
          </div>
