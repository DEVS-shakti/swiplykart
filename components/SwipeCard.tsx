...
export function SwipeCard({ product, saved, onSave, canSave, userId }: SwipeCardProps) {
  ...
  const showCompare = typeof product.compareAtPrice === "number" && product.compareAtPrice > product.price;
  ...
  return (
    <div className="theme-panel relative flex h-full w-full flex-col overflow-hidden rounded-[34px] shadow-[0_40px_90px_rgba(0,0,0,0.2)]">
      <Link
        href={detailHref}
      ...
      <div className="pointer-events-none relative z-[2] -mt-6 shrink-0 px-3 pb-3 md:-mt-7 md:px-4 md:pb-4">
        <div className="theme-panel-strong rounded-[28px] p-5 shadow-[0_20px_50px_rgba(0,0,0,0.22)] md:p-6">
          ...
          <div className="mt-3 flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <h2 className="mt-1 line-clamp-2 font-headline text-2xl font-extrabold leading-tight text-foreground md:text-3xl">
                {title}
              </h2>
            </div>
            <div className="shrink-0 text-right">
              ...
              <div className="rounded-2xl border border-primary/25 bg-primary/12 px-3 py-2">
                ...
                {showCompare ? (
                  <p className="text-xs text-soft-foreground line-through">{formatPrice(product.compareAtPrice!)}</p>
                ) : null}
              </div>
            </div>
          </div>
          ...
          <p className="mt-3 line-clamp-2 text-sm leading-relaxed text-soft-foreground">{product.description}</p>
          <div className="pointer-events-auto mt-4 grid grid-cols-2 gap-3">
            <button
              ...
              className={cn(
                "flex items-center justify-center gap-2 rounded-2xl border py-3.5 text-sm font-bold transition",
                saved
                  ? "border-tertiary/30 bg-tertiary/20 text-tertiary"
                  : "border-outline/70 bg-surface-container-low/85 text-foreground hover:bg-surface-container",
            ...
            <button
              type="button"
              onClick={buyNow}
              className="flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-secondary-container to-primary py-3.5 text-sm font-bold text-white shadow-[0_0_28px_rgba(109,11,236,0.25)] transition hover:opacity-95"
            >
